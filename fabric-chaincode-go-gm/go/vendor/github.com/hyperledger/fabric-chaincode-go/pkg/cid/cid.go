...
func GetX509Certificate(stub ChaincodeStubInterface) (*sm2.Certificate, error) {
	c, err := New(stub)
	if err != nil {
		return nil, err
	}
...
func (c *ClientID) GetID() (string, error) {
	// When IdeMix, c.cert is nil for sm2 type
	// Here will return "", as there is no sm2 type cert for generate id value with logic below.
	if c.cert == nil {
		return "", fmt.Errorf("cannot determine identity")
	}
	// The leading "sm2::" distinguishes this as an sm2 certificate, and
	// the subject and issuer DNs uniquely identify the sm2 certificate.
	// The resulting ID will remain the same if the certificate is renewed.
	id := fmt.Sprintf("sm2::%s::%s", getDN(&c.cert.Subject), getDN(&c.cert.Issuer))
...
func (c *ClientID) HasOUValue(OUValue string) (bool, error) {
	x509Cert := c.cert
	if x509Cert == nil {
		// Here it will return false and an error, as there is no sm2 type cert to check for OU values.
		return false, fmt.Errorf("cannot obtain an sm2 certificate for the identity")
	}
...
// Getsm2Certificate returns the sm2 certificate associated with the client,
// or nil if it was not identified by an sm2 certificate.
func (c *ClientID) GetX509Certificate() (*sm2.Certificate, error) {
	return c.cert, nil
}
...
func (c *ClientID) init() error {
	...
	if block == nil {
		...
		if err != nil {
			return fmt.Errorf("identity bytes are neither sm2 PEM format nor an idemix credential: %s", err)
		}
		return nil
	}
...
// https://go-review.googlesource.com/c/go/+/67270/1/src/crypto/sm2/pkix/pkix.go#26
