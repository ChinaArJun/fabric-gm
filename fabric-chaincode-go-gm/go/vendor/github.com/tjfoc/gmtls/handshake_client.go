...
func (hs *clientHandshakeState) doFullHandshake() error {
	...
	if c.handshakes == 0 {
		...
		for i, asn1Data := range certMsg.certificates {
			fmt.Printf("asn1Data---,%v",asn1Data)
		...
		if !c.config.InsecureSkipVerify {
			...
			fmt.Printf("certs[0]----,%v",certs[0])
			fmt.Printf("c.config.RootCAs---,%v",c.config.RootCAs)
			fmt.Printf("c.config.ServerName---,%v",c.config.ServerName)
		...
		fmt.Printf("c.peerCertificates ---,%v",c.peerCertificates )
	} else {
