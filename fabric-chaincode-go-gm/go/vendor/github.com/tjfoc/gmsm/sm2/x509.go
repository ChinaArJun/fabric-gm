...
func (c *Certificate) CheckSignature(algo SignatureAlgorithm, signed, signature []byte) error {
	fmt.Printf("this is error 1")
	fmt.Printf("PublicKey",c.PublicKey)
	...
	case *ecdsa.PublicKey:
		...
		case P256Sm2():
			...
			if !Sm2Verify(sm2pub, signed, nil, ecdsaSig.R, ecdsaSig.S) {
				fmt.Printf("valid---,%v", false)
				fmt.Printf("pub.X---,%v", pub.X)
				fmt.Printf("pub.Y---,%v", pub.Y)
				fmt.Printf("pub---,%v", pub)
				fmt.Printf("sm2pub",reflect.TypeOf(sm2pub))
				return errors.New("x509: SM2 verification failure")
			}
			fmt.Printf("valid---,%v", true)
